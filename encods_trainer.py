import gen_enco_dataset as DL
from encods_config import config
import encods_network as net
...
class trainer:
    ...
    def renew_everything(self):
        # renew dataloader.
        self.loader = DL.dataEncoLoader(config)
        self.loader.renew(min(floor(self.resl), self.max_resl))
        
        # define tensors
        self.z = torch.FloatTensor(self.loader.batchsize, self.nz)#check 
    ...
    def train(self):
        ...
        for step in range(2, self.max_resl+1+5):
            for iter in tqdm(range(0,(self.trns_tick*2+self.stab_tick*2)*self.TICK, self.loader.batchsize)):
                ...
                self.x.data = self.feed_interpolated_input(sample_batch['image'])
                ...
                if self.globalIter%self.config.save_img_every == 0:
                    ...
                    utils.save_image_grid(x_test.data, 'repo_enco/save/grid/{}_{}_G{}_D{}.jpg'.format(int(self.globalIter/self.config.save_img_every), self.phase, self.complete['gen'], self.complete['dis']))
                    os.system('mkdir -p repo_enco/save/resl_{}'.format(int(floor(self.resl))))
                    utils.save_image_single(x_test.data, 'repo_enco/save/resl_{}/{}_{}_G{}_D{}.jpg'.format(int(floor(self.resl)),int(self.globalIter/self.config.save_img_every), self.phase, self.complete['gen'], self.complete['dis']))
